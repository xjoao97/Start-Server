    ...
    internal class RideHorseEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (Type)
            {
                if (Pet.RidingHorse)
                {
                    var Speech2 = OblivionServer.GetGame()
                        .GetChatManager()
                        .GetPetLocale()
                        .GetValue("pet.alreadymounted");
                ...
                else
                {
                    ...
                    Room.SendMessage(Room.GetRoomItemHandler()
                        .UpdateUserOnRoller(Pet, new Point(NewX2, NewY2), 0,
                            Room.GetGameMap().SqAbsoluteHeight(NewX2, NewY2)));
                    Room.SendMessage(Room.GetRoomItemHandler()
