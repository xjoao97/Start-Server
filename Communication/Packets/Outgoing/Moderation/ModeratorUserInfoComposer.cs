    ...
    internal class ModeratorUserInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
            ...
            WriteInteger(User != null
                ? Convert.ToInt32(
                    Math.Ceiling((OblivionServer.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) / 60))
                : 0);
            WriteInteger(User != null
                ? Convert.ToInt32(
                    Math.Ceiling((OblivionServer.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60))
                : 0);
            WriteBoolean(User != null &&
                         OblivionServer.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) !=
                         null);
            ...
            WriteString(Convert.ToDouble(Info["trading_locked"]) != 0 ? origin.ToString("dd/MM/yyyy HH:mm:ss") : "0");
            //Trading lock
