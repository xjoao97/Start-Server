        ...
        public void OnTrigger(GameClient Session, Item Item, int Request, bool HasRights)
        {
            ...
            if (!(Math.Abs(User.X - Item.GetX) >= 2 || Math.Abs(User.Y - Item.GetY) >= 2))
            {
                    ...
                    Room.GetGameMap().CanRollItemHere(NewPoint.X, NewPoint.Y))
                {
                    ...
                    mMessage.LoadList(Outgoing.ObjectOnRoller); // Cf
                    ...
                    Room.SendMessage(new SlideObjectBundleComposer(Item.GetX, Item.GetY, Item.GetZ, NewPoint.X,
                        NewPoint.Y, NewZ, 0, 0, Item.Id));
