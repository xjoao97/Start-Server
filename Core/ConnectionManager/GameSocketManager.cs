...
//        private int maximumConnections;
        ...
        public void init(int portID, int maxConnections, int connectionsPerIP, IDataParser parser,
...
//            maximumConnections = maxConnections;
            ...
            log.Info("Sucesso, GameSocketManager na porta (" + portID + ")!");
            ...
            if (connectionListener != null)
                if (_acceptConnections)
                    try
                    {
                        ...
                        {
                            log.Info("Conexão negada de [" +
                                     replyFromComputer.RemoteEndPoint.ToString().Split(':')[0] +
                                     "]. Muitas conexões (" + ConnectionCount + ").");
                        }
                    }
                    catch
                    {
        ...
        private void reportUserLogin(string ip) => alterIpConnectionCount(ip, getAmountOfConnectionFromIp(ip) + 1);

        /// <summary>
        ///     reports the user with an ip as "logged out"
        /// </summary>
        /// <param name="ip">The ip of the user</param>
        private void reportUserLogout(string ip) => alterIpConnectionCount(ip, getAmountOfConnectionFromIp(ip) - 1);
